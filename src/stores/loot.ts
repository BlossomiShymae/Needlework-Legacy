...
export const useLootStore = defineStore('loot', {
  ...
  getters: {
    championCapsules: (state) => state.playerLootMap.get("CHEST_128"),
    gloriousChampionCapsules: (state) => state.playerLootMap.get("CHEST_129"),
    lootChampions: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.CHAMPION
    ),
    lootEternals: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.ETERNALS
    ),
    lootChests: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.CHEST
    ),
    lootEmotes: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.EMOTE
    ),
    lootSkins: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.SKIN
    ),
    lootIcons: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.SUMMONER_ICON
    ),
    lootWardSkins: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.WARD_SKIN
    ),
    lootTacticians: (state) => filterMapByCategory(
      state.playerLootMap, Loot.DisplayCategories.COMPANION
    ),
